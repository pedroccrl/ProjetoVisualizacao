...
//radio buttons
var radio_calor = document.getElementById('radio_calor');
var radio_pontos = document.getElementById('radio_pontos');
...
btn_select.addEventListener("click", function(e){
    ...
    //Reinicio do mapa
    initMap();
    if (radio_calor.checked) {
        VisualizaoCalor();
    }
    else if(radio_pontos.checked){
        VisualizaoPontos();
    }
    else {
        alert("Escolha uma opção de visualizaçao");
        avisos.innerHTML = "Opção de visualização inválida";
    }
});

function VisualizaoCalor() {
    if (select_api.options[select_api.selectedIndex].value == "Socrata")
